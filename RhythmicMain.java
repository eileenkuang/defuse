...
public class RhythmicMain {
	...
	public GeneralButton begin = new GeneralButton(820, 300, 150, 60, beginImage,
			beginHover);
...
//				System.out.println("index: " + index + " + max: " + max);
				...
				if(seqOrNot && index < max) {
					...
					if(max == 1) {
						sequence[index]=(int)(Math.random()*4+1);
					}
					else{
						...
						if(sequence[max-1] == sequence[max-2]) {
					...
					System.out.println("index " + index + ": "+ sequence[index]);
	...
	// when user presses the sequence wrong, plays music and regenerates a new pattern
	public void lose() {
